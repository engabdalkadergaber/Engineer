...
(function () {
  ...
  function initParticles(canvas) {
    ...
    function resize() {
      ...
      // Guard against zero dimensions on mobile before layout settles
    ...
    window.addEventListener("orientationchange", () => {
      setTimeout(() => { cancelAnimationFrame(rafId); if (resize()) step(); }, 250);
    });
      ...
      requestAnimationFrame(() => {
        requestAnimationFrame(() => {
          if (resize()) {
            ...
            // Retry after a short delay if canvas still has zero dimensions
    ...
    if (document.fonts && document.fonts.ready) {
      document.fonts.ready.then(attemptBoot);
    }
    // Also boot on load as a safety net so mobile always starts
    window.addEventListener("load", () => {
      if (!hasBooted) attemptBoot();
    });
    // Final safety net: try once DOM is ready
